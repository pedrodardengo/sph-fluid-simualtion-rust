...
impl FluidSimulationApp {
    ...
    pub fn update(&mut self) {
        ...
        self.accelerations.par_iter_mut().enumerate().for_each(|(index, acceleration)| {
                let adjacente_particles_indices_iterator = self
                    .cell_manager
                    .get_adjacent_particles_indices(self.particles[index].position);
                ...
                new_acceleration += self
                    .external_attractor
                    .get_external_attraction_acceleration(&self.particles[index], self.local_densities[index]);
