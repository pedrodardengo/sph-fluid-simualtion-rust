...
impl FluidSimulationApp {
    pub fn new(box_dimensions: [usize; 2]) -> Self {
        ...
        let pressure_multiplier: f32 = 110000.0;
        let target_density: f32 = 0.00002;
        let smoothing_radius: f32 = 16.0;
    ...
    pub fn update(&mut self) {
        ...
        let densities: Vec<f32> = (0..self.particles.len())
            ...
            .map(|particle_index| {
                let adjacente_particles_indices_iterator = self
                    .cell_manager
                    .get_adjacent_particles_indices(self.particles[particle_index].position);
        ...
        let accelerations: Vec<Vector2D<f32>> = (0..self.particles.len())
            .into_par_iter()
            .map(|particle_index| {
                let adjacente_particles_indices_iterator = self
                    .cell_manager
                ...
                acceleration += self
                    .external_attractor
                    .get_external_attraction_acceleration(&self.particles[particle_index]);
                acceleration
            })
            .collect();
        ...
        if window.get_mouse_down(MouseButton::Left) {
            ...
            if let Some(mouse_position) = mouse_position {
                self.external_attractor
                    .activate(Vector2D::new(mouse_position.0, mouse_position.1));
            }
        }
        if !window.get_mouse_down(MouseButton::Left) {
